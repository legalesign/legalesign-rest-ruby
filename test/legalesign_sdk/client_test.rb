...
class LegalesignSDKTest < Minitest::Test
  ...
  def test_raises_on_missing_non_nullable_opts
    e = assert_raises(ArgumentError) do
      LegalesignSDK::Client.new
    end
    assert_match(/is required/, e.message)
  end

  def test_client_default_request_default_retry_attempts
    stub_request(:get, "http://localhost/document/").to_return_json(status: 500, body: {})

    legalesign_sdk = LegalesignSDK::Client.new(base_url: "http://localhost", api_key: "My API Key")

    assert_raises(LegalesignSDK::Errors::InternalServerError) do
      legalesign_sdk.document.list(group: "group")
    end
  ...
  def test_client_given_request_default_retry_attempts
    ...
    legalesign_sdk =
      LegalesignSDK::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 3)
  ...
  def test_client_default_request_given_retry_attempts
    ...
    assert_raises(LegalesignSDK::Errors::InternalServerError) do
      legalesign_sdk.document.list(group: "group", request_options: {max_retries: 3})
    end
  ...
  def test_client_given_request_given_retry_attempts
    ...
    assert_raises(LegalesignSDK::Errors::InternalServerError) do
      legalesign_sdk.document.list(group: "group", request_options: {max_retries: 4})
    end
  ...
  def test_client_retry_after_seconds
    stub_request(:get, "http://localhost/document/").to_return_json(
      status: 500,
    ...
    legalesign_sdk =
      LegalesignSDK::Client.new(base_url: "http://localhost", api_key: "My API Key", max_retries: 1)
  ...
  def test_client_redirect_307
    ...
    assert_raises(LegalesignSDK::Errors::APIConnectionError) do
      legalesign_sdk.document.list(group: "group", request_options: {extra_headers: {}})
    end

    recorded, = WebMock::RequestRegistry.instance.requested_signatures.hash.first

    assert_requested(:any, "http://localhost/redirected", times: LegalesignSDK::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_303
    ...
    assert_requested(:get, "http://localhost/redirected", times: LegalesignSDK::Client::MAX_REDIRECTS) do
  ...
  def test_client_redirect_auth_strip_cross_origin
    ...
    assert_requested(:any, "https://example.com/redirected", times: LegalesignSDK::Client::MAX_REDIRECTS) do
      headers = _1.headers.keys.map(&:downcase)
  ...
  def test_default_headers
    stub_request(:get, "http://localhost/document/").to_return_json(status: 200, body: {})
