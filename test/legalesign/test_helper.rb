...
class Legalesign::Test::SingletonClient < Legalesign::Client
  include Singleton

  TEST_API_BASE_URL = ENV.fetch("TEST_API_BASE_URL", "http://localhost:4010")

  def initialize
    super(base_url: Legalesign::Test::SingletonClient::TEST_API_BASE_URL, api_key: "My API Key")
  end
end
...
class Legalesign::Test::ResourceTest < Minitest::Test
  def async?
  ...
  def before_all
    super
    @legalesign = Legalesign::Test::SingletonClient.instance
  end
